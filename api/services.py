# api_logistica/services.py
import requests
from fastapi import HTTPException
from typing import List, Optional
from . import config
from .schemas import CarrierInfo, CarrierListResponse, ConfigInfo

def get_carriers_from_envia() -> List[CarrierInfo]:
    """
    ...
    if not config.TOKEN:
        raise HTTPException(
            status_code=500,
            detail="Token de la API de Envia.com no está configurado"
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Convertir a nuestros esquemas y categorizar
            carriers = []
            ...
            raise HTTPException(
                ...
                detail=f"Error al obtener carriers de Envia.com: {response.text}"
            )
    
    except requests.exceptions.RequestException as e:
        raise HTTPException(
            status_code=503,
            detail=f"No se pudo conectar con la API de Envia.com: {str(e)}"
        )

def get_carrier_by_name(carrier_name: str) -> Optional[CarrierInfo]:
    """
